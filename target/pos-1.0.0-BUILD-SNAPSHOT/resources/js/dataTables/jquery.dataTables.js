...
(/** @lends <global> */function( $ ) {
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		var jqFilter = $('input[type="search"]', filter)
			...
			.bind( 'keypress.DT', function(e) {
				...
				if ( e.keyCode == 13 ) {
					var val = !this.value ? "" : this.value; // mental IE8 fix :-(
					fnCallbackFire( settings, 'aoSearchDone', 'val', [val] );
					return false;
				}
			} )
	...
	function _fnFeatureHtmlLength ( settings )
	{
		...
		select
			...
			.bind( 'change.DT', function(e) {
				var aPreSelect = _fnCallbackFire( settings, 'aoPreSelectLength', 'length', [$(this).val()] );
				if ( $.inArray( false, aPreSelect ) !== -1 )
				{
					return;
				}
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			_fnCallbackReg( oSettings, 'aoPreSelectLength',    oInit.fnPreSelectLength,   'user' );
			_fnCallbackReg( oSettings, 'aoSearchDone',   	   oInit.fnSearchDone,   'user' );
	...
	DataTable.defaults = {
		 ...
		 *      $('#example').dataTable( {
		 *        "stateSave": true,
		 *        "fnPreSelectLength": function (val) {
		 *          data.oSearch.sSearch = "";
		 *        }
		 *      } );
		 *    } );
		 */
		"fnPreSelectLength":null,
